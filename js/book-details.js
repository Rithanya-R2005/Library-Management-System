...
// Load book data from catalog page
...
// Update book details with data from catalog
function updateBookDetails(book) {
    ...
    if (breadcrumbSpan) {
        breadcrumbSpan.textContent = book.title;
    }
    
    // Update book title
    const bookTitle = document.querySelector('.book-header h1');
    if (bookTitle) {
        bookTitle.textContent = book.title;
    }
    
    // Update author
    const authorElements = document.querySelectorAll('.book-meta .meta-item:has(label:contains("Author")) span, .book-meta .meta-item span');
    authorElements.forEach(element => {
        if (element.previousElementSibling && element.previousElementSibling.textContent === 'Author') {
            element.textContent = book.author;
        }
    });
    
    // Update ISBN
    const isbnElements = document.querySelectorAll('.book-meta .meta-item:has(label:contains("ISBN")) span, .book-meta .meta-item span');
    isbnElements.forEach(element => {
        if (element.previousElementSibling && element.previousElementSibling.textContent === 'ISBN') {
            element.textContent = book.isbn;
        }
    });
    
    // Update subject
    const subjectElements = document.querySelectorAll('.book-meta .meta-item:has(label:contains("Subject")) span, .book-meta .meta-item span');
    subjectElements.forEach(element => {
        if (element.previousElementSibling && element.previousElementSibling.textContent === 'Subject') {
            element.textContent = book.subject;
        }
    });
